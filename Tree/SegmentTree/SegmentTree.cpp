...
public:
    ...
    void ConstructTree(const std::vector<int>& arr, int low, int high, int pos)
    {
        if (low == high)
        {
            tree[pos] = arr[low];
    ...
    int _Query(int qlow, int qhigh, int low, int high, int pos)
    {
    ...
    void _Update(int index, int diff, int low, int high, int pos)
    {
...
int main()
{
    ...
    std::cout << "Сумма на отрезке [1, 3]: " << segTree.Query(1, 3) << std::endl;

    segTree.Update(2, 10);

    std::cout << "Сумма на отрезке [1, 3] после обновления: " << segTree.Query(1, 3) << std::endl;
